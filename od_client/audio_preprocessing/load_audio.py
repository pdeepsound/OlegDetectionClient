...
def load_wav(
    ...
    Args:
        ...
        max_duration: the max duration in seconds from start of the audio clip to load
    ...
    if sr < 8000:
        raise ValueError("Sampling rate is too low, please use audio clips with sampling rate >= 8000")
    ...
    if channel < channels:
        if channels != 1:
            audio_bytes = np.frombuffer(audio_bytes, dtype=dtype)[channel::channels].tobytes()
    else:
        raise ValueError("The number of channels in the audio clip must be greater than channel + 1")
